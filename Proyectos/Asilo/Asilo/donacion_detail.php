...
<body data-spy="scroll" data-target=".site-navbar-target" data-offset="300">
    ...
    function sendMail($Pname, $IMGname, $fname, $lname, $phone, $email, $message, $myemail, $mypassword)
    {
        $mail = new PHPMailer();
        $mail->isSMTP();
        ...
        $mail->Body = "<b>Name:</b> {$fname}<br><b>Apellido:</b> {$lname}<br><br><b>Numero de telefono:</b> {$phone}<br><br><b>Mail:</b> {$email}<br><br><b>Message:</b><br><br>{$message}:</b><br><br>{$Pname}:</b><br><br>{$IMGname}";
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($fname) || empty($lname) || empty($phone) || empty($email) || empty($date) || empty($hora) || empty($message)) {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if (!preg_match('/^\+?[0-9\s]{7,13}$/', $phone)) {
                    ...
                    // Consultar la base de datos para obtener las credenciales del correo electrónico
                    $resultado = mysqli_query($con, "SELECT email, token FROM configmail WHERE id='1'");
                    if ($resultado->num_rows > 0) {
                        $row = mysqli_fetch_assoc($resultado);
                        $myemail = $row['email'];
                        $mypassword = $row['token'];

                        // Obtener los valores de $Pname y $IMGname de la base de datos
                        $consulta = "SELECT DonacionTitle, Image FROM tbldonacion WHERE ID = '$id'";
                        $resultadoDonacion = mysqli_query($con, $consulta);
                        if ($filaDonacion = mysqli_fetch_assoc($resultadoDonacion)) {
                            $Pname = $filaDonacion['DonacionTitle'];
                            $IMGname = $filaDonacion['Image'];

                            // Insertar datos en la tabla correspondiente
                            $query = mysqli_query($con, "INSERT INTO tblDoncontact (Producto, Imagen, FirstName, LastName, Phone, Email, date, hora,  Message) VALUES ('$Pname', '$IMGname', '$fname', '$lname', '$phone', '$email', '$date', '$hora', '$message')");
                            if ($query) {
                                // Si la inserción fue exitosa, enviar el correo electrónico
                                if (sendMail($Pname, $IMGname, $fname, $lname, $phone, $email,  $message, $myemail, $mypassword)) {
                                    echo "<p><script>Swal.fire({ title: 'ÉXITO', text: 'Registro exitoso y correo enviado', icon: 'success', button: 'Cerrar' }).then(() => { window.location.href = 'donacion1.php'; });</script></p>";
                                } else {
                            ...
                            echo "<p><script>Swal.fire({ title: 'ERROR', text: 'No se encontraron datos de donación', icon: 'warning', button: 'Cerrar' });</script></p>";
                        }
                    } else {
                ...
                <div class="row">
                    <div class="col-lg-8 mb-5">
                        <form class="forms-sample" method="post">
                            <?php
                            $id = intval($_GET['id']);
                            $query = mysqli_query($con, "SELECT * FROM tbldonacion WHERE tbldonacion.ID='$id'");

                            while ($row = mysqli_fetch_array($query)) {
                                ?>
                                <div class="form-group">
                                    <h1 class="display-4 text-left my-4" id="DonacionTitle">
                                        <?php echo htmlentities($row['DonacionTitle']); ?>
                                    </h1>
                                </div>

                                <div class="form-group text-left">
                                    <label class="text-black font-weight-bold" for="Image">Imagen</label>
                                    <br>
                                    <img id="Image" src="admin/images/<?php echo htmlentities($row['Image']); ?>"
                                        class="img-fluid rounded shadow-sm mt-3" alt="Donacion Image" width="400">
                                </div>
                            <?php } ?>

                            <div class="row form-group">
                                <div class="col-md-12">
                                    <h4 class="text-black" for="fname">Escribe cómo deseas ayudarnos</h4>
                                    <br>
                                </div>
                            ...
                            <div class="row form-group">
    <div class="col-md-12">
        <label class="text-black" for="fecha-limite">Fecha límite</label>
        <input type="date" id="fecha-limite" name="date" class="form-control">
    </div>
</div>

                            <div class="row form-group">
                                <div class="col-md-12">
                                    <label class="text-black" for="Hora">Hora</label>
                                    <input type="time" name="hora" id="hora" min="08:00" max="17:00" class="form-control">
                                </div>
                            </div>
                            ...
                            <div class="row form-group">
                                <div class="col-md-12">
                                    ...
                                    <textarea name="message" id="message" cols="30" rows="7" maxlength="350"
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <div class='col-lg-3 ml-auto'>
                                ...
                                <img src='Refugio_Logo.png' alt='Imagen del refugio' style='width: 50px; height: auto;'>
                                <br><br>
...
function obtenerFechaLimite() {
    ...
    return new Date(fechaActual.getFullYear(), fechaActual.getMonth() + 3, fechaActual.getDate());
}
...
function actualizarFechaEnPagina() {
    const fechaActual = obtenerFechaActual();
    ...
    // Establecer la fecha máxima como un año a partir de hoy
    inputFecha.max = formatearFecha(fechaLimite);

    // Establecer el valor por defecto como la fecha límite
    inputFecha.value = inputFecha.max;
}

    // Actualizar la fecha al cargar la página
    document.addEventListener('DOMContentLoaded', actualizarFechaEnPagina);
    </script>
        </br>
